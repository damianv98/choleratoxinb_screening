...
compound_df = pd.read_csv("top1000_1PZJ_smiles.txt", sep=' ', header=None)
...
# Generate image
img = Draw.MolsToGridImage(
 [compounds[clusters[i][0]][0] for i in range(0, 8)],
 legends=[f"Cluster {i}" for i in range(1, 9)],
 subImgSize=(250, 250),
 useSVG=True,
)
...
with open("cluster_representatives_1PZJ_8.svg", "w") as f:
 f.write(molsvg)
...
for d, i in enumerate(cluster_mols):
    with open('cluster' + str(d+1) + '_1PZJ.txt', 'w') as outfile:
        for j in i:
            outfile.write(j+"\n")
